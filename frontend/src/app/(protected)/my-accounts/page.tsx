...
// Basic-user facing accounts list page.
// Dark themed layout to match the protected area.
export default function MyAccountsPage() {
  ...
  // Spec 17: All users see all accounts - no filtering

  // Redirect Admin users to /accounts since they should use the Admin Accounts page
  useEffect(() => {
    if (user?.role === "Admin") {
  ...
  // UI-only account size label from number of users
  ...
  const handleOpen = (id: string) => {
    router.push(`/my-accounts/${id}`);
  };

  const handleEdit = (id: string) => {
    const acct = accounts.find((a) => a.id === id);
    if (!acct || !user) return;
    if (acct.createdByUserId !== user.id) {
      if (typeof window !== "undefined" && typeof window.alert === "function") {
        window.alert("Only Admin can access this action.");
      }
      return;
    }
    router.push(`/my-accounts/${id}?edit=1`);
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                onClick={() => router.push("/my-accounts/new")}
              >
          ...
          {!loading && !error && accounts.length === 0 && (
            <div className="px-4 py-10 text-center sm:px-6">
              ...
              <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                Once you or your team add accounts, they will appear here with quick access to their details.
              </p>
            </div>
          )}
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                <thead className="bg-slate-50 dark:bg-gray-900">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right text-sm font-medium tracking-wider text-gray-500 dark:text-gray-400">
                      Actions
                    </th>
                  </tr>
                </thead>
                        ...
                        <div className="relative inline-block text-left overflow-visible" onMouseDown={(e)=>e.stopPropagation()}>
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              const menuHeight = 88; // approx height
                              const gap = 8;
